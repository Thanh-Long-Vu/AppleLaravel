                    ...
                    <div class="product-item__inner bg-white p-3">
                        <div class="product-item__body pb-xl-2">
                            ...
                            <h5 class="mb-1 product-item__title">
                                <a href="{{ route('categories.show', ['category' => $item->category->id_category]) }}"
                                    class="text-blue font-weight-bold">
                                    {{ $item->name }}-
                                    <b style="color: {{ $colors[(int) $item->products->first()->warehouse->color]['color'] ?? 'grey' }}; "
                                        class="mx-1">
                            ...
                            <div class="mb-2">
                                <a href="{{ route('products.show', ['product' => $item->products->first()->id_product ?? 0]) }}"
                                    class="d-block text-center"><img class="img-fluid"
                                        src="../{{ $item->products->first()->thumbnail }}"
                            ...
                            <div class="flex-center-between mb-1">
                                @if ($item->products->first()->discount > 0)
                                    <div class="prodcut-price d-flex align-items-center position-relative">
                                        <ins
                                            class="font-size-20 text-red text-decoration-none"><b>${{ number_format($item->products->first()->price - $item->products->first()->price * ($item->products->first()->discount / 100)) }}</b></ins>
                                        <del
                                            class="font-size-12 tex-gray-6 position-absolute bottom-100">${{ number_format($item->products->first()->price) }}</del>
                                    </div>
                                @else
                                    <div class="prodcut-price">
                                        <div class="text-gray-100">
                                            <b>${{ number_format($item->products->first()->price) }}</b>
                                        </div>
                                    </div>
                                @endif
                                @if ($item->products->first()->active_quantity !== null && ($item->products->first()->active_quantity - $item->products->first()->quantity_sell) > 0 && $item->products->first()->active_quantity != $item->products->first()->quantity_sell)
                                    ...
                                    <div class="d-none d-xl-block prodcut-add-cart">
                                        <a onclick="addCart({{ $item->products->first()->id_product }})"
                                            href="javascript:" class="btn-add-cart btn-primary transition-3d-hover"><i
